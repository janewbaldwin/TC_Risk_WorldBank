...
for i in range(np.shape(lon)[1]):
    ...
    iTlandfall_forwindfield_phi[i] = [iT for iT in iTlandfall_forwindfield_phi[i] if iT not in np.where(np.isnan(lon[:,i]))[0]] # remove any other nan values (this is done based on wspd for ibtracs data, but can't be here due to ensemble members)
