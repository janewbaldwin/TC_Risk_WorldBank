...
def windfield_sym(lon_nS,lat_nS,wspd_nS,rmax_nS,i):
    ...
    radius_max = 500 # km
    radius_precision = 1 # km
    profile = W_profile(lati, rmaxi, wspdi, radius_max, radius_precision) # rmax in km, wspdi in m/s
...
Y = np.arange(0,25.1,0.1)#np.arange(-80,80.1,0.1)
...
missed_tries = 0
for nS in np.arange(435,436,1):#len(wspd_landfall),1):
    ...
    rmax_nS[np.where(rmax_nS<0)] = 20 # 6-12-20: set places with radii less than 0 to 20 km to fix convergence
    ...
    try:
        ...
        #Write to netcdf
        ds.to_netcdf('/data2/jbaldwin/WINDFIELDS/PHI_REG_TEST/wspd_phi_'+str(nS)+'.nc',mode='w',unlimited_dims = ["nS"])
    except:
        missed_tries += 1
